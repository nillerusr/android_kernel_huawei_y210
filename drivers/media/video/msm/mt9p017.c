...
static int mt9p017_reset_camera_esd(void)
{
    int rc = 0;

    const struct msm_camera_sensor_info *data = mt9p017_ctrl->sensordata;
    if(data == NULL)
        return -EINVAL;
    ...
    rc = mt9p017_write_exp_gain(3616,3376);

    printk(KERN_DEBUG "%s:%d rc = %d\n",__func__,__LINE__,rc);
                                ...
                                struct msm_sensor_ctrl *             s)
{
    ...
    s->s_reset_sensor_esd = mt9p017_reset_camera_esd;
