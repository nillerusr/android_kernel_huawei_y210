...
static struct s5k5ca_i2c_reg_conf s5k5ca_init_reg_config_sunny_settings_patch[] =
{
...
int s5k5ca_sensor_open_init(const struct msm_camera_sensor_info *data)
{
    ...
    else
    {
        ...
        if(true == s5k5ca_init_flag)
        {
            //reinit some regs by FAE
            rc = s5k5ca_i2c_write_w_table(s5k5ca_init_reg_config_sunny_settings_patch,
                sizeof(s5k5ca_init_reg_config_sunny_settings_patch)/sizeof(s5k5ca_init_reg_config_sunny_settings_patch[0]));
            ...
            //skip some frames: drop some yellow frames
            msleep(400);// 6 frames
        }
        goto init_done;
    }
...
/*add s5k5ca reset function, it will be called when framethread timeout*/
static int s5k5ca_reset_camera_esd(void)
{
    int rc = 0;

    const struct msm_camera_sensor_info *data = s5k5ca_ctrl->sensordata;
    if(data == NULL)
    {
        return -EINVAL;
    }
                               ...
                               struct msm_sensor_ctrl *             s)
{
    ...
    s->s_reset_sensor_esd = s5k5ca_reset_camera_esd;
