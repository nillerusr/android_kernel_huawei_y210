...
static int msm_reset_camera_esd(struct msm_sync *sync,void __user *arg)
{
	...
	if(sync->sctrl.s_reset_sensor_esd)
	{
		...
		printk(KERN_DEBUG "%s: try_count=%d\n",__func__,esd_debug_count);

		rc = sync->sctrl.s_reset_sensor_esd();
		if(rc < 0)
			printk("sync->sctrl.s_reset_regs fail\n");
