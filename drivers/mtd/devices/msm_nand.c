...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	if (!flash_onfi_probe(chip)) {
		...
		if (!flashdev->pagesize) {
			...
			/* Changed for nand which meanings of id is not the same with the others.
			 * Hynix H9DA4VH2GJMMCR-4EM (id 0x1190bcad) id4 all bit.
			 * Hynix H9DA4VH2GJAMCR-4EM (id 0x1680bcad) id4 all bit.
			 * Samsung K524G2GACH-B050  (id 0x6a00bcec) id4 bit 3~2.
			 */
			if ((0x1190bcad == flash_id) || (0x1680bcad == flash_id) || (0x6a00bcec == flash_id)) {
				supported_flash.widebus = 1;
				supported_flash.pagesize = 4096;
				supported_flash.blksize = (256 * 1024);
